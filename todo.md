...
- ~~**투표 기능**~~(✅ 2026-08-26 완료) — 사용자와 설계 논의 후 서로 다른 두 기능으로 나눠 확정:
  (1) 인기 게시글 주간 콘테스트, (2) 게시글/한마디 첨부형 설문. 상세 설계는 계획 파일 참고, 아래는
  실제 구현 요약.
  - **인기 게시글 주간 콘테스트**(`post.domain.PostContestEntry`/`PostContestVote`,
    `post.service.PostContestService`, `post.controller.PostContestController`,
    `/posts/contest`): 매주(월~일) 본인 게시물만 회차당 1개 후보 신청(`(nominator_id,
    week_start)` 유니크 제약), 회차당 인당 1표(본인 후보 투표 금지, `(voter_id, week_start)`
    유니크 제약). 매주 월요일 자정 `@Scheduled(cron = "0 0 0 * * MON")`
    `tallyPreviousWeek()`가 득표 상위 3명에게 30/20/10점 지급 — 신설한
    `UserPointService.awardBonus()`로 일일 획득 한도(30점)를 건너뛰어 그날 활동으로 한도를
    이미 채웠어도 우승 보상은 항상 전액 지급됨. `Notification.Type.CONTEST_WIN` 신규 추가.
    cron을 임시로 10초 주기로 바꿔 실제 스케줄 실행→집계→포인트 지급→알림까지 DB로 직접 확인
    후 원래 주기로 복귀(`user_point_logs`에 "주간 인기 게시글 1위" +30 기록, 알림 정상 수신
    확인). **알려 둘 것**: `Notification.type`이 여전히 MySQL 네이티브 ENUM 컬럼인데
    (CLAUDE.md 기존 함정 - REPLY 추가 때는 수동 ALTER TABLE이 필요했음) 이번엔 `ddl-auto:
    update`가 `CONTEST_WIN` 값 추가를 예외적으로 자동 반영했음(로컬에서 `DESCRIBE
    notifications`로 확인) — 다른 환경(운영 DB 등)에서도 그런지는 배포 전 별도 확인 필요.
  - **게시글/한마디 첨부형 설문**(신규 최상위 패키지 `poll` - `Post`/`ScheduleComment` 양쪽에
    붙어 어느 한쪽 패키지에도 종속시키지 않음, `notice`가 `post`와 분리된 것과 같은 판단):
    질문 + 옵션(2~10개), 복수선택 허용 여부·"기타"(투표자가 직접 옵션 추가) 허용 여부·익명
    투표 여부(누가 뭘 골랐는지 숨김, `PollService.buildResult()`가 결과 DTO 변환 시점에
    리댁션 - DB엔 항상 투표자를 남겨 중복투표 방지) 전부 작성자가 개별 설정. 결과 공개범위는
    독립 옵션(같은 학교·학년·반 기본값 / 같은 학년(학교 한정 여부 선택) / 전체공개-링크전용,
    `Post.Visibility.UNLISTED`와 같은 개념) - 작성자 본인과 관리자는 항상 통과. 투표는 단일/
    복수선택 모두 "새로 고른 것으로 교체" 방식(QnA 답변 채택의 "새 채택 시 기존 자동 해제"와
    동일한 원자적 교체 패턴, delete 후 flush로 유니크 제약 충돌 방지). 게시글 작성 폼(`post/
    form.html`)과 한마디 작성 폼(`school/comment-form.html`)에 동일한 설문 첨부 섹션을
    추가(수정 화면에는 없음 - 이미 투표가 쌓인 설문을 고치는 흐름은 범위 밖), Quill 리치
    에디터 임베드로 넣지 않고 별도 구조화 폼 필드로 분리(CLAUDE.md의 "Quill dangerouslyPasteHTML
    함정" 참고 - 설문은 실시간 투표 상태를 가진 구조화 데이터라 정적 HTML 직렬화 모델과 안
    맞음). 결과/투표 위젯(`static/js/poll.js`)은 게시글 상세·캘린더 한마디 목록에서
    `GET /polls/by-post/{id}` 또는 `/polls/by-comment/{id}`로 자기 데이터를 스스로 불러오는
    위젯 패턴(알림 배지 폴링과 동일 컨벤션) - 설문이 없으면 204로 조용히 숨김. 실사용 시나리오로
    작성→투표→득표 반영, 공개범위 밖 계정으로 접근 시 차단, 중복투표 차단(`이미 투표했습니다`)
    까지 브라우저/curl로 end-to-end 확인.
  - 남은 것(이번 라운드 범위 밖): 콘테스트 과거 우승 이력 조회 화면, 설문 결과를 관리자
    화면에서 모아보는 기능, 콘테스트 시상 알림 외 별도 리마인더(마감 임박 알림 등).
