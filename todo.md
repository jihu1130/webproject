...
## -3. 버그 리포트 상세 페이지 (2026-08-24 요청, 0순위)
...
- [ ] `AdminBugReportController`에 `GET /admin/bug-reports/{id}` 상세 조회 추가
- [ ] `admin/bug-report-detail.html` 신규 템플릿(공지 상세 페이지 패턴 참고) - 첨부 이미지/영상은
      상세 페이지에서 더 크게 보여줄 것(목록의 40px 썸네일보다)
- [ ] 목록의 제목 셀을 상세 페이지로 가는 링크로 변경(`admin/notice-list.html`의 제목 링크 패턴과
      동일)
- [ ] 해결됨 표시/삭제 액션은 상세 페이지에서도 가능하게(목록에서 이동 안 하고 바로 처리 가능한
      지금 방식도 유지)
