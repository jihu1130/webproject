...
- 🔐 **로그인/회원가입** — 로컬 계정 + 구글 소셜 로그인(OAuth2), 첫 로그인 시
  학교 설정 강제 온보딩, 5회 연속 실패 시 계정 잠금(브루트포스 방지),
  탈퇴 후에도 같은 구글 계정으로 재로그인하면 자동 복구(단, 관리자가 강제
  탈퇴시킨 계정은 예외)
...
- 🏆 **인기 게시글 주간 콘테스트** — 본인 게시물 후보 신청, 득표 상위 3명에게
  매주 포인트 지급
- 🛍️ **포인트 · 티어 · 상점** — 활동으로 포인트를 모아 상점에서 프로필 효과
  (반짝임/무지개 등) 구매·장착
...
- 🧾 **관리자 감사 로그** — 누가 언제 무엇을 처리했는지 총관리자가 조회·필터링
...
| Build | Gradle |
| Infra / CI·CD | AWS EC2 · S3(파일 저장) · SSM, GitHub Actions (OIDC 기반 배포) |
| Monitoring | UptimeRobot(가동 확인) |
...
├── poll         : 게시글/한마디에 붙는 설문·투표
├── notice       : 공지사항(관리자 작성, 활성 공지 1개 유지, 이력 보관)
├── notification : 댓글/좋아요/관리자 조치/공지 알림
├── bugreport    : 버그 제보 · 1:1 문의 및 관리자 답변
└── admin        : 관리자 행동 감사 로그(총관리자 전용)
```
...
| `ROLE_SUPER_ADMIN` | 총관리자 — 여러 명 지정 가능, 부관리자 권한 승격/회수, 감사 로그 열람 |
