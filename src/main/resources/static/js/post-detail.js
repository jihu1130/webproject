...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ---- 추천 ----
    var postRecommendBtn = document.getElementById('postRecommendBtn');
    if (postRecommendBtn) {
        postRecommendBtn.addEventListener('click', function () {
            WebSchoolRequestGuard.run(postRecommendBtn, function () {
                return fetch('/posts/contest/posts/' + postId + '/recommend', { method: 'POST', headers: WebSchoolCsrf.headers() })
                    .then(function (res) {
                        if (!res.ok) {
                            return res.json().then(function (body) { throw new Error(body.error || '추천에 실패했습니다.'); });
                        }
                        postRecommendBtn.disabled = true;
                        postRecommendBtn.textContent = '추천완료';
                    ...
                    .catch(function (err) {
                        WebSchoolModal.alert(err.message || '추천에 실패했습니다.');
                    });
            });
        });
    }
