...
public class AuthController {
    ...
    public String register(@ModelAttribute RegisterDto registerDto,
        ...
        try {
            ...
            // 버그 수정: 예전엔 학교/학년/반까지 다 입력해 가입을 마쳐도 로그인 페이지로 보내져서
            // 아이디/비밀번호를 또 입력해야 했다 - 가입 직후 바로 로그인 상태로 만들어준다.
    ...
    // 비밀번호는 UserService.register()가 이미 검증/저장했으니 다시 인증받을 필요 없이 UserDetails를
    // 직접 로드해서 SecurityContext에 채워 넣는다. SecurityContextHolder에만 설정하면 이 요청이
    // 끝나는 순간 사라지므로, HttpSessionSecurityContextRepository로 세션에도 명시적으로 저장해야
    // 리다이렉트 이후 요청에서도 로그인 상태가 유지된다.
